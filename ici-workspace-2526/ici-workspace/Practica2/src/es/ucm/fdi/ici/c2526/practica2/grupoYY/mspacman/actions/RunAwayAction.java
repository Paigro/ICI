...
public class RunAwayAction implements Action {
	int minDist;
	/**
	 * @param minDist Distancia minima para ser considerada una zona segura.
	 */
	...
	/**
	 * Un nodo es seguro si no contiene un fantasma y no esta demasiado cerca.
	 */
	private boolean isSafeNode(Game game, int node, List<Integer> ghostNodes) {
	    for (GHOST ghost : GHOST.values()) {
	        int ghostNode = game.getGhostCurrentNodeIndex(ghost);
	        if (ghostNode == -1 || game.isGhostEdible(ghost)) continue;
	        double dist = game.getShortestPathDistance(node, ghostNode);
	        MOVE ghostMove = game.getGhostLastMoveMade(ghost);
	        int ghostNext = game.getNeighbour(ghostNode, ghostMove);
	        
	        // Si esta muy cerca o viene hacia el nodo, peligro.
	        if (dist < 10.0 || ghostNext == node)
	            return false;
	    }
	    return true;
	}
