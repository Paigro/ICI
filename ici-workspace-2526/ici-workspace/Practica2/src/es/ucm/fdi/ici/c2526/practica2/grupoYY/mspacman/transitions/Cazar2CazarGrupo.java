...
public class Cazar2CazarGrupo implements Transition {
    int minDistance;
    int minGhostToGroup;
    /**
     ...
     */
	public Cazar2CazarGrupo(int minDistance, int minGhostToGroup) {
		this.minDistance = minDistance;
		this.minGhostToGroup = minGhostToGroup;
		}
        ...
        for (Map.Entry<GHOST,Integer> e1 : ghostPos.entrySet()) {
            ...
            for (Map.Entry<GHOST,Integer> e2 : ghostPos.entrySet()) {
				...
				if(m.getGame().getDistance(pos1, pos2, DM.PATH) <= this.minDistance 
					...
					nGhosts++;
				if(nGhosts >= this.minGhostToGroup) return true;
			}		
		}
	...
	public String toString() {
		return String.format("Se ha encontrado un grupo de fantasmas");
	}
}
